...
if [ $# -lt 3 ]
  then
        ...
        echo "Usage: ./uploadWPA.sh <interface>"  
else
        BSSID=$1
        CHAN=$2
        INTERF=$3
	...
	# Gets interface name (might have changed)
	MON=$(iwconfig 2> /dev/null | grep -i mon|awk '{print $1}')
        DATE=`date '+%Y_%m_%d_%H_%M_%S'`
	KEY="KEY-$BSSID-$DATE.txt" 
        LOG_FILE=log-$BSSID-$DATE.txt
        sudo airodump-ng -w $BSSID --write-interval 1 -c $CHAN --bssid $BSSID $MON &> /dev/null & #output-format csv
	pid_airodump=$!
	...
	# "dictionary" to keep track of the networks already cracked
        while [ ! -f $KEY ]; do #while true:
		# Look for PWR > -85
